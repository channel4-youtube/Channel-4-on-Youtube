...
class channel_4_youtube(TVShowIndexer, TVShowSource):
    implements = [TVShowIndexer, TVShowSource]

    name = 'Youtube: Channel 4'
    display_name = 'Youtube: Channel 4'
    
    img = 'http://yt3.ggpht.com/-cc9bEUdU9fo/AAAAAAAAAAI/AAAAAAAAAAA/sh4nam8-m9M/s256-c-k-no/photo.jpg'
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        if section == 'main':
            self.AddSection(list, indexer, 'uploads', 'Uploads', 'http://gdata.youtube.com/feeds/api/users/channel4/uploads?&v=2&max-results=50&alt=jsonc', indexer)
            self.AddSection(list, indexer, 'playlists', 'Playlists', 'http://gdata.youtube.com/feeds/api/users/channel4/playlists?&v=2&max-results=50&alt=jsonc', indexer)
        else:
