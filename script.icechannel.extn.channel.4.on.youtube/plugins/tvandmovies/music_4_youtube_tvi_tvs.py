...
class music_4_youtube(TVShowIndexer, TVShowSource):
    implements = [TVShowIndexer, TVShowSource]

    name = 'Youtube: 4 Music'
    display_name = 'Youtube: 4 Music'
    
    img = 'http://yt3.ggpht.com/-s8sP7AWGx5M/AAAAAAAAAAI/AAAAAAAAAAA/XpOI6fmiF90/s256-c-k-no/photo.jpg'
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        if section == 'main':
            self.AddSection(list, indexer, 'uploads', 'Uploads', 'http://gdata.youtube.com/feeds/api/users/TheOfficial4Music/uploads?&v=2&max-results=50&alt=jsonc', indexer)
            self.AddSection(list, indexer, 'playlists', 'Playlists', 'http://gdata.youtube.com/feeds/api/users/TheOfficial4Music/playlists?&v=2&max-results=50&alt=jsonc', indexer)
        else:
