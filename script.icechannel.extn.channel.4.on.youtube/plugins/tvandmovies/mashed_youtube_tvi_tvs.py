...
class mashed_youtube(TVShowIndexer, TVShowSource):
    ...
    name = 'Youtube: Mashed'
    display_name = 'Youtube: Mashed'
    
    img = 'http://yt3.ggpht.com/-Rx3tC3dMQxc/AAAAAAAAAAI/AAAAAAAAAAA/0drB4oNxyvQ/s256-c-k-no/photo.jpg'
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        if section == 'main':
            self.AddSection(list, indexer, 'uploads', 'Uploads', 'http://gdata.youtube.com/feeds/api/users/mashed/uploads?&v=2&max-results=50&alt=jsonc', indexer)
            self.AddSection(list, indexer, 'playlists', 'Playlists', 'http://gdata.youtube.com/feeds/api/users/mashed/playlists?&v=2&max-results=50&alt=jsonc', indexer)
        else:
