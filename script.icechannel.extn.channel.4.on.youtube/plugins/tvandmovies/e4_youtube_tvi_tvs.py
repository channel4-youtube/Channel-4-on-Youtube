...
class mashed_youtube(TVShowIndexer, TVShowSource):
    ...
    name = 'Youtube: E4'
    display_name = 'Youtube: E4'
    
    img = 'http://yt3.ggpht.com/-dEFl_NeYhh4/AAAAAAAAAAI/AAAAAAAAAAA/U8nLRr7oAeU/s256-c-k-no/photo.jpg'
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        if section == 'main':
            self.AddSection(list, indexer, 'uploads', 'Uploads', 'http://gdata.youtube.com/feeds/api/users/e4/uploads?&v=2&max-results=50&alt=jsonc', indexer)
            self.AddSection(list, indexer, 'playlists', 'Playlists', 'http://gdata.youtube.com/feeds/api/users/e4/playlists?&v=2&max-results=50&alt=jsonc', indexer)
        else:
