...
class channel_4_racing_youtube(TVShowIndexer, TVShowSource):
    implements = [TVShowIndexer, TVShowSource]

    name = 'Youtube: Channel 4 Racing'
    display_name = 'Youtube: Channel 4 Racing'
    
    img = 'http://yt3.ggpht.com/--qu128URFL0/AAAAAAAAAAI/AAAAAAAAAAA/KhCvVje1beY/s256-c-k-no/photo.jpg'
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        if section == 'main':
            self.AddSection(list, indexer, 'uploads', 'Uploads', 'http://gdata.youtube.com/feeds/api/users/Channel4Racing/uploads?&v=2&max-results=50&alt=jsonc', indexer)
            self.AddSection(list, indexer, 'playlists', 'Playlists', 'http://gdata.youtube.com/feeds/api/users/Channel4Racing/playlists?&v=2&max-results=50&alt=jsonc', indexer)
        else:
