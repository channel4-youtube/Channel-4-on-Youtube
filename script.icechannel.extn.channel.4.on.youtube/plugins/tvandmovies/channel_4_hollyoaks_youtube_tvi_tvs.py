...
class channel_4_hollyoaks_youtube(TVShowIndexer, TVShowSource):
    implements = [TVShowIndexer, TVShowSource]

    name = 'Youtube: Channel 4\'s Hollyoaks'
    display_name = 'Youtube: Channel 4\'s Hollyoaks'
    
    img = 'http://yt3.ggpht.com/-uroLhxHYZ6M/AAAAAAAAAAI/AAAAAAAAAAA/RR3ei89RDtw/s256-c-k-no/photo.jpg'
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        if section == 'main':
            self.AddSection(list, indexer, 'uploads', 'Uploads', 'http://gdata.youtube.com/feeds/api/users/hollyoaks/uploads?&v=2&max-results=50&alt=jsonc', indexer)
            self.AddSection(list, indexer, 'playlists', 'Playlists', 'http://gdata.youtube.com/feeds/api/users/hollyoaks/playlists?&v=2&max-results=50&alt=jsonc', indexer)
        else:
