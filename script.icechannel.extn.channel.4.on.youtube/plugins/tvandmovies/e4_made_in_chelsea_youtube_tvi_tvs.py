...
class mashed_youtube(TVShowIndexer, TVShowSource):
    implements = [TVShowIndexer, TVShowSource]

    name = 'Youtube: E4\'s Made in Chelsea'
    display_name = 'Youtube: E4\'s Made in Chelsea'
    
    img = 'http://yt3.ggpht.com/-TDkpa79D2Ro/AAAAAAAAAAI/AAAAAAAAAAA/me6qIfTWX9g/s256-c-k-no/photo.jpg'
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        if section == 'main':
            self.AddSection(list, indexer, 'uploads', 'Uploads', 'http://gdata.youtube.com/feeds/api/users/e4madeinchelsea/uploads?&v=2&max-results=50&alt=jsonc', indexer)
            self.AddSection(list, indexer, 'playlists', 'Playlists', 'http://gdata.youtube.com/feeds/api/users/e4madeinchelsea/playlists?&v=2&max-results=50&alt=jsonc', indexer)
        else:
