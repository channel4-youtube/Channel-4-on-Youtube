...
class docs_on_4_youtube(TVShowIndexer, TVShowSource):
    implements = [TVShowIndexer, TVShowSource]

    name = 'Youtube: Docs on 4'
    display_name = 'Youtube: Docs on 4'
    
    img = 'http://yt3.ggpht.com/-jQd2dicQauY/AAAAAAAAAAI/AAAAAAAAAAA/FoW9USbxPkI/s256-c-k-no/photo.jpg'
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        if section == 'main':
            self.AddSection(list, indexer, 'uploads', 'Uploads', 'http://gdata.youtube.com/feeds/api/users/ooTDp-fERMGRHIQ4VL4qlg/uploads?&v=2&max-results=50&alt=jsonc', indexer)
            self.AddSection(list, indexer, 'playlists', 'Playlists', 'http://gdata.youtube.com/feeds/api/users/ooTDp-fERMGRHIQ4VL4qlg/playlists?&v=2&max-results=50&alt=jsonc', indexer)
        else:
